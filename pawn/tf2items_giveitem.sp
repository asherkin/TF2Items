...
public Action:Command_WeaponEx(client, args)
{
	...
	weaponIndex = StringToInt(arg1);
	
	new weaponSlot;
	new String:formatBuffer[32];
	Format(formatBuffer, 32, "%d_%s", weaponIndex, "slot");
	GetTrieValue(g_hItemInfoTrie, formatBuffer, weaponSlot);
...
public Action:Command_Weapon(client, args)
{
	...
	new Handle:hWeapon = PrepareItemHandle(weaponIndex);
...
Handle:PrepareItemHandle(weaponIndex) {
	new String:formatBuffer[32];	
	...
	Format(formatBuffer, 32, "%d_%s", weaponIndex, "classname");
	GetTrieString(g_hItemInfoTrie, formatBuffer, weaponClassname, 32);
	
	Format(formatBuffer, 32, "%d_%s", weaponIndex, "slot");
	GetTrieValue(g_hItemInfoTrie, formatBuffer, weaponSlot);
	
	Format(formatBuffer, 32, "%d_%s", weaponIndex, "quality");
	GetTrieValue(g_hItemInfoTrie, formatBuffer, weaponQuality);
	
	Format(formatBuffer, 32, "%d_%s", weaponIndex, "level");
	GetTrieValue(g_hItemInfoTrie, formatBuffer, weaponLevel);
	
	Format(formatBuffer, 32, "%d_%s", weaponIndex, "attribs");
